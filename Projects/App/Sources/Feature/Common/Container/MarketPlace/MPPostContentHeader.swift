...
public struct MPPostContentHeader {
  ...
  private let pointAmount: Int // 햇살 포인트
  private let meetingPlace: String // 활동 장소
  
  public init(avatar: URL?, userName: String, locationName: String, createdAt: Date, postStatus: PostStatus, postType: PostType, title: String, content: String, pointAmount: Int, meetingPlace: String) {
    self.avatar = avatar
  ...
  public init(_ vm: ViewModel){
    self.avatar = vm.avatar
    self.userName = vm.userName
    self.locationName = vm.locationName
    self.createdAt = vm.createdAt
    self.postStatus = vm.postStatus
    self.postType = vm.postType
    self.title = vm.title
    self.content = vm.content
    self.pointAmount = vm.pointAmount
    self.meetingPlace = vm.meetingPlace
  }
  
  public struct ViewModel: Equatable {
    let avatar: URL? // 캐릭터 이미지 url
    
    let userName: String // 유저이름
    var locationName: String // 동네
    var createdAt: Date // 글 작성 날짜
    
    let postStatus: PostStatus
    let postType: PostType
    
    let title: String // 제목
    let content: String // 본문 콘텐츠
    
    let pointAmount: Int // 햇살 포인트
